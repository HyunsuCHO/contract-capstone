...
import "./Counters.sol";
import {ERC4907CustomUpgradable} from "../contracts/ERC4907CustomUpgradable.sol";

contract RentNFTOpenland2 is ERC4907CustomUpgradable {
    ...
    function startRenting(uint256 tokenId, uint256 months) public payable {
        require(months >= 3, "3 months at least");
        checkIsAllowedToken(tokenId);
        checkPayment(tokenId, months);

        // 대여 기간 설정 및 사용자 설정
        uint64 expires = uint64(block.timestamp + months * 30 days);
    ...
    // 중도 취소 함수
    function cancelRenting(uint256 tokenId) public {
